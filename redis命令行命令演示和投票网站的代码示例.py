...
# 通过使用复制特性来扩展读性能，还可以使用客户端分片来扩展写性能。

# 高性能键值缓存服务器memcached

# Redis既可以用作主数据库（primary database），又可以作为其他存储系统的辅助数据库（auxiliary database）使用。

# 使用内存存储 in-memory

# 批量操作bulk operation 和不完全（partial）的事物支持
...
# 随机读（random read）随机写（random write） 关系型数据库行的更新操作

# 原子的（atomic） INCR 命令

# 发送给Redis的命令请求并不需要经过典型的查询分析器（parser）或者查询优化器（optimizer）进行处理，所以对Redis
* 存储的数据执行随机写的速度总是非常迅速的。

# 工具会极大地改变人们解决问题的方式

# https://redis.io/commands  命令列表
...
# 这些散列表只有键，但没有与键相关联的值。
# smembers命令获取集合包含的所有元素（如果包含的元素非常多，那么smembers命令的执行速度可能会很慢，所以请谨慎使用这个命令）
...
# 代码清单 1-4
# 散列存储的值既可以是字符串又可以是数字值，并执行自增操作或者自减操作
# 散列在很多方面就像是一个微缩版的Redis，不少字符串命令都有相应的散列版本。
# 散列类似文档数据库里面的文档或者关系数据库里面的行。因为这三者都允许用户同时
# 访问或者修改一个或者多个域（field）。
# 散列拥有数据存储能力
...
# 代码清单 1-5
# 有序集合和散列一样，都用于存储键值对：有序集合的键被称为成员（member），每个
# 成员都是各不相同的；而有序集合的值则被称为分值（score），分值必须为浮点数。
# 有序集合是Redis里面唯一一个既可以根据成员访问元素（这一点和散列一样），又可以
# 根据分值以及分值的排列顺序来访问元素的结构。
# 有序集合内建排序能力
...
# 代码清单 1-6
# todo 对调投票：比如将支持票转换成反对票，或者将反对票转换为支持票。可以参考smove这个命令
...
def get_articles(conn, page, order='score:'):
    ...
    # 获取多个文章ID。
    # 有序集合会根据成员分值从小到大地排列元素
    # 而使用zrevrange命令则是以"分值从大到小"的排列顺序
